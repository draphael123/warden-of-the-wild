...
export default function Home(){
  ...
  const reset=useCallback(()=>{
    ...
    game.current={gold:260,lives:20,wave:0,score:0,state:"ready",towers:[] as Tower[],enemies:[] as Enemy[],falls:[],bolts:[] as Bolt[],particles:[] as Particle[],texts:[] as Popup[],spawn:[] as any[],spawnClock:0,nextId:1,time:0,shake:0,flash:0,message:"Build your first elemental tower.",hover:-1,mouse:{x:500,y:300},selectedTower:-1,speed:1,combo:0,comboTimer:0,best,muted:false,veteran:false,ability:0,abilityShown:0,aiming:false,surgeMark:null};
...
function damage(g:any,e:Enemy,amount:number){e.hp-=Math.max(2,amount-e.armor);if(e.hp<=0&&e.alive){e.alive=false;g.falls??=[];g.falls.push({x:e.x,y:e.y,kind:e.kind,life:e.kind==="warden"?1.1:.65});g.gold+=e.reward;g.score+=e.reward*10*(g.veteran?1.5:1);g.dirty=true;burst(g,e.x,e.y,e.kind==="warden"?"#ffc65c":"#dbe7b6",12);g.shake=e.kind==="warden"?10:2;if(e.kind==="warden")g.texts.push({x:e.x,y:e.y-28,label:"KRAK!",life:1.1,color:"#ffd879"})}}
function update(g:any,dt:number,sync:()=>void){if(!g)return;g.texts??=[];g.falls??=[];g.time+=dt;g.shake=Math.max(0,g.shake-dt*20);g.flash=Math.max(0,g.flash-dt);
  ...
  g.enemies=g.enemies.filter((e:Enemy)=>e.alive);for(const f of g.falls)f.life-=dt;g.falls=g.falls.filter((f:any)=>f.life>0);for(const b of g.bolts)b.life-=dt;g.bolts=g.bolts.filter((b:Bolt)=>b.life>0);for(const p of g.particles){p.x+=p.vx*dt;p.y+=p.vy*dt;p.vy+=45*dt;p.life-=dt}g.particles=g.particles.filter((p:Particle)=>p.life>0);for(const t of g.texts){t.y-=22*dt;t.life-=dt}g.texts=g.texts.filter((t:Popup)=>t.life>0);
...
function enemyArt(c:CanvasRenderingContext2D,e:Enemy,time:number){
  ...
  c.fillStyle="#171914";c.fillRect(-e.radius,-e.radius-12,e.radius*2,5);c.fillStyle=e.hp/e.maxHp<.3?"#ff665b":"#a8de75";c.fillRect(-e.radius,-e.radius-12,e.radius*2*Math.max(0,e.hp/e.maxHp),5);c.restore();
}
