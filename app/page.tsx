...
export default function Home(){
  ...
  const [ui,setUi]=useState({gold:260,lives:20,wave:0,score:0,state:"ready",message:"The Hollow stirs at dusk.",speed:1,combo:0,best:0,muted:false,veteran:false,ability:0,aiming:false,nextReady:false,rushBonus:0});
  ...
  const reset=useCallback(()=>{
    ...
    game.current={gold:260,lives:20,wave:0,score:0,state:"ready",towers:[] as Tower[],enemies:[] as Enemy[],falls:[],bolts:[] as Bolt[],particles:[] as Particle[],texts:[] as Popup[],rings:[],spawn:[] as any[],spawnClock:0,nextId:1,time:0,shake:0,flash:0,message:"Build your first elemental tower.",hover:-1,mouse:{x:500,y:300},selectedTower:-1,speed:1,combo:0,comboTimer:0,best,muted:false,veteran:false,ability:0,abilityShown:0,aiming:false,surgeMark:null};
  ...
  useEffect(()=>{reset();try{setAutoWaves(localStorage.getItem("warden-auto-waves")==="true")}catch{/* Storage is optional. */}},[reset]);
  ...
  useEffect(()=>{if(!autoWaves||ui.state!=="between"||ui.wave>=waves.length){setAutoCountdown(0);return}setAutoCountdown(10);const started=Date.now(),ticker=window.setInterval(()=>setAutoCountdown(Math.max(0,Math.ceil(10-(Date.now()-started)/1000))),200),timer=window.setTimeout(()=>startWave(),10000);return()=>{clearInterval(ticker);clearTimeout(timer)}}
      ...
      <div className="stage-wrap">
        ...
        {panel&&<div className="menu-overlay" role="dialog" aria-modal="true" aria-label={panel==="guide"?"Field Guide":panel==="settings"?"Settings":"Level Select"}>
          <section className="folio">
            ...
            {panel==="settings"&&<><span className="folio-kicker">BATTLE OPTIONS</span><h2>Settings</h2><div className="settings-list"><button onClick={()=>{game.current.muted=!game.current.muted;game.current.message=game.current.muted?"Sound muted":"Sound restored";sync()}}><span><b>Sound</b><small>Battle tones and alerts</small></span><strong>{ui.muted?"OFF":"ON"}</strong></button><button onClick={()=>{game.current.veteran=!game.current.veteran;game.current.message=game.current.veteran?"Veteran mode enabled":"Standard mode restored";sync()}}><span><b>Difficulty</b><small>Veteran: tougher enemies, 50% score bonus</small></span><strong>{ui.veteran?"VETERAN":"STANDARD"}</strong></button><button onClick={toggleAutoWaves}><span><b>Auto waves</b><small>Begin the next assault after a 10-second respite</small></span><strong>{autoWaves?"ON":"OFF"}</strong></button><button onClick={()=>{game.current.speed=game.current.speed===2?1:2;game.current.menuSpeed=game.current.speed;sync()}}><span><b>Default battle speed</b><small>Can also be changed during combat</small></span><strong>{(game.current?.menuSpeed??1)===2?"2×":"1×"}</strong></button></div><p className="setting-note">Keyboard: 1–4 select towers · Space calls a wave · R aims Wild Surge · P pauses · Esc closes menus.<br/>Reduced motion follows your device accessibility preference automatically.</p></>}
          </section>
        </div>}
      </div>
...
function damage(g:any,e:Enemy,amount:number){const dealt=Math.max(2,amount-e.armor);e.hp-=dealt;e.hit=.12;g.rings??=[];if(dealt>=28)g.texts.push({x:e.x,y:e.y-10,label:`${Math.round(dealt)}`,life:.48,color:"#fff4c2"});if(e.hp<=0&&e.alive){e.alive=false;g.falls??=[];g.falls.push({x:e.x,y:e.y,kind:e.kind,life:e.kind==="warden"?1.1:.65});g.gold+=e.reward;g.score+=e.reward*10*(g.veteran?1.5:1);g.dirty=true;burst(g,e.x,e.y,e.kind==="warden"?"#ffc65c":"#dbe7b6",12);g.rings.push({x:e.x,y:e.y,color:e.kind==="warden"?"#ffb35c":"#e9e2b0",life:.38,max:.38,size:e.kind==="warden"?55:24});g.shake=e.kind==="warden"?10:2;if(e.kind==="warden")g.texts.push({x:e.x,y:e.y-28,label:"KRAK!",life:1.1,color:"#ffd879"})}}
function update(g:any,dt:number,sync:()=>void){if(!g)return;g.texts??=[];g.falls??=[];g.rings??=[];g.time+=dt;g.shake=Math.max(0,g.shake-dt*20);g.flash=Math.max(0,g.flash-dt);
  ...
  for(const t of g.towers){t.cooldown-=dt;if(t.cooldown>0)continue;const targets=g.enemies.filter((e:Enemy)=>e.alive&&dist(t,e)<t.range).sort((a:Enemy,b:Enemy)=>t.priority==="strong"?b.hp-a.hp:b.waypoint-a.waypoint);const e=targets[0];if(!e)continue;const power=(16+t.level*8)*(t.element==="storm"?.82:1);let reaction="";
    ...
    if(t.element==="storm"){for(const x of targets.slice(0,2+t.level)){damage(g,x,power*(x===e?1:.58));g.bolts.push({x:t.x,y:t.y,tx:x.x,ty:x.y,color:colors.storm,life:.15,kind:"storm"})}if(e.root>0){reaction="OVERGROWTH ARC";for(const x of targets.slice(0,5))damage(g,x,power*.65)}}
    if(t.element!=="storm")g.bolts.push({x:t.x,y:t.y,tx:e.x,ty:e.y,color:colors[t.element],life:.18,kind:t.element});burst(g,e.x,e.y,colors[t.element],3);g.rings.push({x:e.x,y:e.y,color:colors[t.element],life:.22,max:.22,size:18});if(reaction){g.combo=Math.min(9,(g.combo||0)+1);g.comboTimer=2.5;g.message=`${reaction} · ×${g.combo}`;g.score+=25*g.combo;g.dirty=true;g.texts.push({x:Math.max(105,Math.min(W-105,e.x)),y:e.y-18,label:`${reaction}  ×${g.combo}`,life:.9,color:"#fff0a0"});g.rings.push({x:e.x,y:e.y,color:"#fff2a6",life:.55,max:.55,size:58});g.shake=Math.max(g.shake,5);burst(g,e.x,e.y,"#fff5bd",18)}t.cooldown=(t.element==="fire"?.78:t.element==="frost"?.7:t.element==="storm"?1.05:.83)/(1+(t.level-1)*.17)
  }
  g.enemies=g.enemies.filter((e:Enemy)=>e.alive);for(const f of g.falls)f.life-=dt;g.falls=g.falls.filter((f:any)=>f.life>0);for(const b of g.bolts)b.life-=dt;g.bolts=g.bolts.filter((b:Bolt)=>b.life>0);for(const r of g.rings)r.life-=dt;g.rings=g.rings.filter((r:any)=>r.life>0);for(const p of g.particles){p.x+=p.vx*dt;p.y+=p.vy*dt;p.vy+=45*dt;p.life-=dt}g.particles=g.particles.filter((p:Particle)=>p.life>0);for(const t of g.texts){t.y-=22*dt;t.life-=dt}g.texts=g.texts.filter((t:Popup)=>t.life>0);
...
function enemyArt(c:CanvasRenderingContext2D,e:Enemy,time:number){
  const step=Math.sin(time*(e.kind==="wisp"?6:10)+e.x*.08),ink="#20261f";c.save();c.translate(e.x,e.y+step*2);c.lineJoin="round";c.lineCap="round";
  ...
  c.fillStyle="#171914";c.fillRect(-e.radius,-e.radius-14,e.radius*2,6);c.fillStyle=e.hp/e.maxHp<.3?"#ff665b":"#a8de75";c.fillRect(-e.radius,-e.radius-14,e.radius*2*Math.max(0,e.hp/e.maxHp),6);c.restore();
}
