...
type Tower = { x:number; y:number; element:Element; level:number; cooldown:number; range:number; id:number };
type Enemy = { x:number; y:number; waypoint:number; hp:number; maxHp:number; speed:number; armor:number; radius:number; burn:number; burnTick:number; chill:number; root:number; poison:number; alive:boolean; kind:string; reward:number };
...
function tone(kind:"build"|"wave"|"upgrade"){
  try{audio??=new AudioContext();const now=audio.currentTime,osc=audio.createOscillator(),gain=audio.createGain();osc.connect(gain);gain.connect(audio.destination);osc.type=kind==="wave"?"sawtooth":"sine";osc.frequency.setValueAtTime(kind==="build"?260:kind==="upgrade"?420:120,now);osc.frequency.exponentialRampToValueAtTime(kind==="build"?520:kind==="upgrade"?780:220,now+.18);gain.gain.setValueAtTime(.0001,now);gain.gain.exponentialRampToValueAtTime(.07,now+.015);gain.gain.exponentialRampToValueAtTime(.0001,now+.24);osc.start(now);osc.stop(now+.25)}catch{}
}
...
export default function Home(){
  ...
  const [ui,setUi]=useState({gold:260,lives:20,wave:0,score:0,state:"ready",message:"The Hollow stirs at dusk.",speed:1});
  const setElement=(e:Element)=>{setSelected(e);selectedRef.current=e};

  const reset=useCallback(()=>{
    game.current={gold:260,lives:20,wave:0,score:0,state:"ready",towers:[] as Tower[],enemies:[] as Enemy[],bolts:[] as Bolt[],particles:[] as Particle[],spawn:[] as any[],spawnClock:0,nextId:1,time:0,shake:0,flash:0,message:"Build your first elemental tower.",hover:-1,selectedTower:-1,speed:1};
    setUi({gold:260,lives:20,wave:0,score:0,state:"ready",message:"Build your first elemental tower.",speed:1});
  },[]);
  useEffect(()=>reset(),[reset]);

  const sync=()=>{const g=game.current;if(g)setUi({gold:g.gold,lives:g.lives,wave:g.wave,score:g.score,state:g.state,message:g.message,speed:g.speed})};
  const startWave=()=>{const g=game.current;if(!g||g.state==="wave"||g.state==="lost"||g.state==="won")return;if(g.wave>=waves.length)return;tone("wave");g.wave++;g.state="wave";g.message=g.wave===6?"THE ASHEN WARDEN APPROACHES":"Wave "+g.wave+" breaks from the Hollow";g.spawn=[];let delay=.5;waves[g.wave-1].forEach(group=>{for(let i=0;i<group.count;i++){g.spawn.push({...group,at:delay});delay+=group.gap}delay+=1});g.spawnClock=0;sync()};

  const upgrade=()=>{const g=game.current;if(!g||g.selectedTower<0)return;const t=g.towers[g.selectedTower],price=75+t.level*55;if(t.level<3&&g.gold>=price){tone("upgrade");g.gold-=price;t.level++;t.range+=12;g.message=`${names[t.element]} tower awakened to level ${t.level}`;burst(g,t.x,t.y,colors[t.element],16);sync()}};
  ...
  const clickCanvas=(ev:React.MouseEvent<HTMLCanvasElement>)=>{
    ...
    if(pi>=0){const e=selectedRef.current,cost=costs[e];if(g.gold>=cost){tone("build");g.gold-=cost;g.towers.push({x:pads[pi][0],y:pads[pi][1],element:e,level:1,cooldown:0,range:e==="storm"?142:154,id:g.nextId++});g.selectedTower=g.towers.length-1;g.message=`${names[e]} tower bound to the old stone`;burst(g,pads[pi][0],pads[pi][1],colors[e],14)}else g.message="Not enough sunstone";sync()}
  };
  ...
  return <main>
    <section className="game-shell">
      <header><div><span className="eyebrow">ELEMENTAL DEFENSE</span><h1>WARDEN OF THE WILD</h1></div><div className="status"><span>◈ <b>{ui.gold}</b></span><span>♥ <b>{ui.lives}</b></span><span>WAVE <b>{ui.wave}/6</b></span><span>SCORE <b>{ui.score}</b></span></div></header>
      ...
      <footer>
        ...
        <div className="actions">
          {t&&<><div className="selection"><small>SELECTED</small><b style={{color:colors[t.element]}}>{names[t.element]} · LV {t.level}</b></div><button className="secondary" onClick={sell}>SELL</button><button className="secondary" disabled={t.level>=3||ui.gold<upgradeCost} onClick={upgrade}>{t.level>=3?"MAX LEVEL":`UPGRADE ◈${upgradeCost}`}</button></>}
          <button className="secondary speed" onClick={()=>{game.current.speed=game.current.speed===1?2:1;game.current.message=game.current.speed===2?"Time quickens through the Wild":"The Wild returns to its natural rhythm";sync()}}>{ui.speed}× SPEED</button>
          <button className="wave" disabled={ui.state==="wave"||ui.state==="lost"||ui.state==="won"} onClick={startWave}>{ui.wave===0?"CALL FIRST WAVE":ui.wave>=6?"FINAL WAVE":"CALL NEXT WAVE"}<span>Release when your defenses are ready</span></button>
        </div>
      </footer>
...
function damage(g:any,e:Enemy,amount:number){e.hp-=Math.max(2,amount-e.armor);if(e.hp<=0&&e.alive){e.alive=false;g.gold+=e.reward;g.score+=e.reward*10;g.dirty=true;burst(g,e.x,e.y,e.kind==="warden"?"#ffc65c":"#dbe7b6",12);g.shake=e.kind==="warden"?10:2}}
function update(g:any,dt:number,sync:()=>void){if(!g)return;g.time+=dt;g.shake=Math.max(0,g.shake-dt*20);g.flash=Math.max(0,g.flash-dt);
  if(g.state==="wave"){g.spawnClock+=dt;while(g.spawn.length&&g.spawn[0].at<=g.spawnClock){const s=g.spawn.shift();g.enemies.push({x:path[0][0],y:path[0][1],waypoint:1,hp:s.hp,maxHp:s.hp,speed:s.speed,armor:s.kind==="brute"?8:s.kind==="warden"?16:0,radius:s.kind==="warden"?23:s.kind==="brute"?16:11,burn:0,burnTick:0,chill:0,root:0,poison:0,alive:true,kind:s.kind,reward:s.reward})}}
  for(const e of g.enemies){if(!e.alive)continue;e.burn=Math.max(0,e.burn-dt);e.poison=Math.max(0,e.poison-dt);e.chill=Math.max(0,e.chill-dt);e.root=Math.max(0,e.root-dt);if(e.burn>0||e.poison>0){e.burnTick+=dt;if(e.burnTick>.25){damage(g,e,(e.burn>0?4:0)+(e.poison>0?2.5:0));e.burnTick=0}}if(!e.alive)continue;const p=path[e.waypoint];if(!p)continue;const d=Math.hypot(p[0]-e.x,p[1]-e.y),spd=e.root>0?0:e.speed*(e.chill>0?.56:1);if(d<spd*dt+2){e.x=p[0];e.y=p[1];e.waypoint++;if(e.waypoint>=path.length){e.alive=false;g.lives-=e.kind==="warden"?8:e.kind==="brute"?2:1;g.shake=12;g.flash=.2;if(g.lives<=0){g.lives=0;g.state="lost";g.message="The Heartstone has fallen."}sync()}}else{e.x+=(p[0]-e.x)/d*spd*dt;e.y+=(p[1]-e.y)/d*spd*dt}}
  for(const t of g.towers){t.cooldown-=dt;if(t.cooldown>0)continue;const targets=g.enemies.filter((e:Enemy)=>e.alive&&dist(t,e)<t.range).sort((a:Enemy,b:Enemy)=>b.waypoint-a.waypoint);const e=targets[0];if(!e)continue;const power=(16+t.level*8)*(t.element==="storm"?.82:1);let reaction="";
    ...
    if(t.element!=="storm")g.bolts.push({x:t.x,y:t.y,tx:e.x,ty:e.y,color:colors[t.element],life:.18});burst(g,e.x,e.y,colors[t.element],3);if(reaction){g.message=reaction;g.score+=25;g.dirty=true;burst(g,e.x,e.y,"#fff5bd",14)}t.cooldown=(t.element==="fire"?.78:t.element==="frost"?.7:t.element==="storm"?1.05:.83)/(1+(t.level-1)*.17)
  }
  ...
  if(g.state==="wave"&&!g.spawn.length&&!g.enemies.length){if(g.wave>=waves.length){g.state="won";g.message="The Ashen Warden is broken. The Wild endures.";g.score+=g.lives*100}else{g.state="between";g.gold+=55+g.wave*10;g.message=`Wave ${g.wave} cleared · Sunstone recovered`}sync()}
}
...
function draw(canvas:HTMLCanvasElement|null,g:any,selected:Element){if(!canvas||!g)return;const c=canvas.getContext("2d")!;c.save();const sx=(Math.random()-.5)*g.shake,sy=(Math.random()-.5)*g.shake;c.translate(sx,sy);const grad=c.createLinearGradient(0,0,0,H);grad.addColorStop(0,"#17382f");grad.addColorStop(1,"#0d211e");c.fillStyle=grad;c.fillRect(-20,-20,W+40,H+40);
  ...
  g.towers.forEach((t:Tower,i:number)=>{if(g.selectedTower===i){c.fillStyle=colors[t.element]+"12";c.strokeStyle=colors[t.element]+"66";c.beginPath();c.arc(t.x,t.y,t.range,0,Math.PI*2);c.fill();c.stroke()}c.fillStyle="#101815";c.beginPath();c.arc(t.x,t.y,26,0,Math.PI*2);c.fill();c.strokeStyle=colors[t.element];c.lineWidth=3;c.stroke();c.shadowColor=colors[t.element];c.shadowBlur=15;c.fillStyle=colors[t.element];c.beginPath();c.arc(t.x,t.y,9+t.level*2,0,Math.PI*2);c.fill();c.shadowBlur=0;c.fillStyle="#fff";c.font="bold 10px Arial";c.textAlign="center";c.fillText(String(t.level),t.x,t.y+4);c.textAlign="left"});
  for(const e of g.enemies){if(!e.alive)continue;c.fillStyle=e.kind==="warden"?"#3b2720":e.kind==="brute"?"#4b4331":"#27362b";c.beginPath();c.arc(e.x,e.y,e.radius,0,Math.PI*2);c.fill();c.strokeStyle=e.chill>0?colors.frost:e.root>0?colors.nature:e.burn>0?colors.fire:"#aebc99";c.lineWidth=2;c.stroke();if(e.kind==="warden"){c.fillStyle="#ff9e5b";c.fillRect(e.x-5,e.y-5,4,4);c.fillRect(e.x+3,e.y-5,4,4)}c.fillStyle="#111";c.fillRect(e.x-e.radius,e.y-e.radius-9,e.radius*2,4);c.fillStyle=e.hp/e.maxHp<.3?"#ff665b":"#a8de75";c.fillRect(e.x-e.radius,e.y-e.radius-9,e.radius*2*Math.max(0,e.hp/e.maxHp),4)}
